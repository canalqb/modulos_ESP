    ...
    <script>
        ...
        (async function() {
            ...
            if (isSTA) {
                ...
                try {
                    // Tenta carregar o CSS remoto
                    const link = document.createElement('link');
                    link.rel = 'stylesheet';
                    link.href = "https://raw.githubusercontent.com/canalqb/modulos_ESP/main/ambiente/style.css";
                    document.head.appendChild(link);

                    // Tenta carregar o HTML remoto via Fetch
                    const response = await fetch("https://raw.githubusercontent.com/canalqb/modulos_ESP/main/ambiente/index.html");
                    if (response.ok) {
                        const html = await response.text();
                        // Injetar apenas o conteúdo do body para não quebrar scripts locais de API se necessário
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        document.body.innerHTML = doc.body.innerHTML;
                        
                        // Re-executar scripts do HTML injetado
